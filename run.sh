...
add_eval() {
  ...
  local line="$prefix ('$rel','$abs'),"
  if [[ "$comment_level" -eq 2 ]]; then
    EVAL_LINES_PART2+=("$line")
  else
    EVAL_LINES_PART1+=("$line")
  fi
}

# 缩写列表：例如 "# B2I-lr0.00001 : '0727/result-.../merged-B2I-lr0.00001'"
add_abbrev() {
  local tag_lr="$1" rel="$2" comment_level="${3:-1}"
  local prefix="#"
  [[ "$comment_level" -eq 2 ]] && prefix="##"
  local line="$prefix ${tag_lr} : '$rel'"
  if [[ "$comment_level" -eq 2 ]]; then
    ABBR_LINES_PART2+=("$line")
  else
    ABBR_LINES_PART1+=("$line")
  fi
}
...
for PAIR in "${MODEL_PAIRS[@]}"; do
  ...
  # ================= runtime helpers for eval toggle & run ===================
  cat >> "$SCRIPT_FILE" <<'RUNTIME_FUNCS'
...
RUNTIME_FUNCS
  ...
  generate_train() {
    ...
    for MAX in "${samples[@]}"; do
      ...
      # 非 LoRA：I 路径 SFT 结果加入评测列表（Part1）
      if ! is_lora && [[ $TAG == I ]]; then
        add_eval "$OUTDIR" 1
        add_abbrev "I-sft-$(lr_tag_dec "$LR")" "${OUTDIR#$RESULTS_DIR/}" 1
      fi
    done
  }
  ...
  if is_lora; then
    # --------- PART 1: 启用（B2I / I2I）---------
    echo "##### Case merge & eval - PART 1 (enabled): B2I / I2I #####" >> "$SCRIPT_FILE"
    ...
    merge_lora() {
      ...
      for RANK in "${lora_ranks[@]}"; do
        for MAX in "${samples[@]}"; do
          ...
          local REL="${MERGED#$RESULTS_DIR/}"
          local TAG_LR="${TAG}-${LR_TAG}"
          ...
          if [[ -z "$COMMENT_PREFIX" ]]; then
            echo "run_eval_for_relpath \"$REL\" 1" >> "$SCRIPT_FILE"
            add_eval "$MERGED" 1
            add_abbrev "$TAG_LR" "$REL" 1
          else
            echo "${COMMENT_PREFIX}run_eval_for_relpath \"$REL\" 2" >> "$SCRIPT_FILE"
            add_eval "$MERGED" 2
            add_abbrev "$TAG_LR" "$REL" 2
          fi
        done
      done
    }
    ...
    # --------- PART 2: 可选（B2B / I2B）---------
    echo "##### Case merge & eval - PART 2 (optional, commented): B2B / I2B #####" >> "$SCRIPT_FILE"
    for LR in "${learning_rates[@]}"; do
      merge_lora "$B_MODEL" B "$B_MODEL" B "$LR" "#"      # B2B（整段注释）
      merge_lora "$I_MODEL" I "$B_MODEL" B "$LR" "#"      # I2B（整段注释）
    done

  else
    echo "##### SFT delta-merge (PART 1 only) #####" >> "$SCRIPT_FILE"
    # SFT：仅做 B2I
    LR="${learning_rates[0]}"
    LR_DEC="$(to_decimal "$LR")"
    LR_TAG="$(lr_tag_dec "$LR")"
    ...
    echo "run_eval_for_relpath \"$REL\" 1" >> "$SCRIPT_FILE"
    add_eval "$MERGED" 1
    add_abbrev "B2I-${LR_TAG}" "$REL" 1
  fi

  # ================= evaluation & abbrev lists =================
  {
    echo "##### Evaluation list - PART 1 (B2I / I2I) #####"
    for line in "${EVAL_LINES_PART1[@]}"; do
      echo "$line"
    done
    echo ""
    echo "##### Evaluation list - PART 2 (B2B / I2B, commented) #####"
    for line in "${EVAL_LINES_PART2[@]}"; do
      echo "$line"
    done
    echo ""
    echo "##### Abbrev list - PART 1 (B2I / I2I) #####"
    for line in "${ABBR_LINES_PART1[@]}"; do
      echo "$line"
    done
    echo ""
    echo "##### Abbrev list - PART 2 (B2B / I2B, commented) #####"
    for line in "${ABBR_LINES_PART2[@]}"; do
      echo "$line"
    done
    echo ""
    ...
    echo ""
    echo "cd ./opencompass"
    echo "python3 ./run.py ./examples/eval_shadow_202505.py -r 20250727_200010"
    echo ""
  } >> "$SCRIPT_FILE"
...
for PAIR in "${MODEL_PAIRS[@]}"; do
  ...
  if is_lora; then
    ...
    merge_lora() {
      ...
      for RANK in "${lora_ranks[@]}"; do
        for MAX in "${samples[@]}"; do
          ...
          # evaluation：启用的（B2I / I2I）用单 '#'; 注释的（B2B / I2B）用双 '##'
          if [[ -z "$COMMENT_PREFIX" ]]; then
            add_eval "$MERGED" 1
          else
            add_eval "$MERGED" 2
    ...
    {
      echo "##### Part 1: Enabled merges (B2I & I2I) #####"
      echo "# Abbreviations (Part 1)"
      echo "#   B2I: Base -> Instruct"
      echo "#   I2I: Instruct -> Instruct"
      echo ""
    } >> "$SCRIPT_FILE"

    # -------- B2I（自成一段）--------
    { echo "###### B2I ######"; echo ""; } >> "$SCRIPT_FILE"
    for LR in "${learning_rates[@]}"; do
    ...
    {
      echo "##### Part 2: Optional merges (B2B & I2B) [commented] #####"
      echo "# Abbreviations (Part 2)"
      echo "#   B2B: Base -> Base"
      echo "#   I2B: Instruct -> Base"
      echo "# 提示：去掉行首 '#' 即可启用本段命令"
      echo ""
    } >> "$SCRIPT_FILE"

    # -------- B2B（整段注释，自成一段）--------
    { echo "###### B2B (commented) ######"; echo ""; } >> "$SCRIPT_FILE"
    for LR in "${learning_rates[@]}"; do
      merge_lora "$B_MODEL" B "$B_MODEL" B "$LR" "#"
    done
    echo "" >> "$SCRIPT_FILE"

    # -------- I2B（整段注释，自成一段）--------
    { echo "###### I2B (commented) ######"; echo ""; } >> "$SCRIPT_FILE"
    for LR in "${learning_rates[@]}"; do
      merge_lora "$I_MODEL" I "$B_MODEL" B "$LR" "#"
    done
    echo "" >> "$SCRIPT_FILE"
  ...
  # ================= evaluation list =================
  {
    ...
    echo "python3 ./run.py ./eval_shadow_20250727_part1.py -r 20250727200010"
    echo "python3 ./run.py ./eval_quantw_20250727.py -r 20250727200011"
